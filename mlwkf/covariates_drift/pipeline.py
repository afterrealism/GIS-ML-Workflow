...
from mlwkf.data_preparation.utilities import check_projection_in_epsg3577
from mlwkf.data_preparation.pipeline import  create_vector_csv
from mlwkf.utilities import save_config_file
from mlwkf.constants import NON_COVARIATES_FIELDS
from mlwkf.data_preparation.pipeline import create_vrt_datasets
from mlwkf.utilities import read_dataframe_from_csv
from mlwkf.covariates_drift.utilities import get_extent_coordinates, create_predicted_geotiff
from mlwkf.registry import lookup_binary_classifier, safe_parse_list, get_ray_redis_password
...
def run_covariate_drift_pipeline(config_file_path):
    ...
    logging.warning("Running covariate_drift")

    os.environ["MODIN_ENGINE"] = "ray"
    time.sleep(2.0)

    ray.init(address='auto', _redis_password=get_ray_redis_password(), ignore_reinit_error=True, local_mode=False)
    ...
    modeling_function = lookup_binary_classifier(config.get('CovariateDrift', 'modeling_function', fallback='LogisticRegression'))
