...
from mlwkf.registry import get_ray_redis_password
...
def create_predicted_geotiff(area_of_interest, covariates, path_to_trained_model, line_geotiff_folder, merged_geotiff_folder, output_folder, model_function, cpus_per_job, gpu_per_job):
    ray.init(address='auto', _redis_password=get_ray_redis_password(), ignore_reinit_error=True)
    ...
    logging.warning("Wait for the tasks to complete and retrieve the results.")
    line_geotiff_files = ray.get(result_ids)

    logging.warning("Release ray store space")
    del x_array_id
    del covariates_id
    del path_to_trained_model_id
    del line_geotiff_folder_id

    logging.warning("Merge line GeoTIFFs")
    ...
    logging.warning("Predicted_geotiff create at: %s", predicted_geotiff)

    logging.warning("Duration: %s", time.time() - start_time)
    ray.shutdown()

    # remove tmp files
    logging.warning("deleting chunked files.")
    ...
    logging.warning("deleting chunked folder.")
    try:
