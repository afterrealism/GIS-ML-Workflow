...
from mlwkf.feature_extraction.utilities import create_feature_ranking_graph, output_results
...
from mlwkf.registry import lookup_model, lookup_scoring_function, safe_parse_dict, safe_parse_list
...
def run_feature_extraction_pipeline(config_file_path):
    ...
    logging.warning("output_folder: %s", output_folder)
    covariates = [
        Path(covariate_path)
        for covariate_path in safe_parse_list(config.get('Intermediate', 'covariates'))
    ]
    ...
    if algorithm == "FeatureRankingByRandomness":
        ...
        model_function_parameters = safe_parse_dict(config.get('Model', 'parameters', fallback="{}"))
        logging.warning(f"Modeling function being used:{str(model_function)}")
        scoring_function = lookup_scoring_function(config.get('FeatureExtraction', 'scoring_function'))
        ...
        model_function = lookup_model(config.get('Model', 'model_function', fallback="XGBRegressor"))
        model_function_parameters = safe_parse_dict(config.get('Model', 'parameters', fallback="{}"))
        scoring_function = lookup_scoring_function(config.get('FeatureExtraction', 'scoring_function', fallback='mean_absolute_error_scorer'))
