...
from mlwkf.registry import lookup_model, lookup_scoring_functions, safe_parse_list
...
def run_hyper_parameter_optimization_pipeline(config_file_path):
    ...
    scoring_functions = lookup_scoring_functions(config.get('HyperParameterOptimization', 'scoring_functions'))
    hyper_parameters = eval(config.get('HyperParameterOptimization', 'hyper_parameters'))  # eval required: contains Ray Tune sampling functions
